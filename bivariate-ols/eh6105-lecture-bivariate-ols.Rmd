...
ggplot(election_turnout, aes(perhsed, turnoutho)) + 
  ...
  labs(title = "A Scatterplot of State-Level Education and Voter Turnout in the 2016 General Election",
       ...
       caption = "Data: ?election_turnout in {stevedata}.")
...
election_turnout %>%
  ...
  geom_label_repel(aes(label=ifelse(z_perhsed > 0 & z_turnoutho < 0 ,as.character(state),'')), family="Open Sans") +
  geom_label_repel(aes(label=ifelse(z_perhsed < 0 & z_turnoutho > 0 ,as.character(state),'')), family="Open Sans") +
  labs(title = "A Scatterplot of State-Level Education and Voter Turnout in the 2016 General Election",
...
broom::tidy(M1)
broom::tidy(M1) %>% slice(2) -> pershed_info_we_want

# divide the coefficient
pull(pershed_info_we_want[1,2])/
   # over the standard error
  pull(pershed_info_we_want[1,3]) -> t_stat # assign to object
