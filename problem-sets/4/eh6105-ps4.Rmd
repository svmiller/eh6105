...
4. There are four Census regions in the United States in which states are located. There is the Midwest (e.g. Ohio, Illinois), the Northeast (e.g. New York, Massachusetts), the South (e.g. Florida, Georgia), and the more diverse and varied West (e.g. California, Utah). Run a "fixed effects" regression that regresses the `trumpshareperc` variable you created on just the `region` variable included in the data. Interpret the results to me, and *be careful* in how you describe them.
